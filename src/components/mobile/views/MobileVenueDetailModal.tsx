...
export const MobileVenueDetailModal: React.FC<Props> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-white flex flex-col max-w-md mx-auto animate-fade-in shadow-2xl">
      ...
      <div className="flex-none bg-white/90 backdrop-blur-md px-4 py-3 border-b border-slate-200 flex items-center justify-between z-30">
        <button
          onClick={onClose}
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-3 gap-2 my-4">
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 text-center shadow-sm">
              <span className="text-[10px] text-slate-500 uppercase font-semibold text-center block">Cena od</span>
              <p className="text-sm font-extrabold text-brand-700 mt-0.5 whitespace-nowrap">{venue.priceFrom} zł <span className="text-[9px] font-normal text-slate-500">/ os.</span></p>
            </div>
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 text-center shadow-sm">
              <span className="text-[10px] text-slate-500 uppercase font-semibold text-center block">Pojemność</span>
              <p className="text-sm font-extrabold text-amber-700 mt-0.5">do {venue.maxGuests} osób</p>
            </div>
            <div className="bg-slate-50 border border-slate-100 rounded-xl p-2.5 text-center shadow-sm">
              <span className="text-[10px] text-slate-500 uppercase font-semibold text-center block">Zaliczka</span>
              <p className="text-sm font-extrabold text-slate-800 mt-0.5">{selectedPackage.depositPercent}%</p>
            </div>
          </div>

          {/* Tabs Navigation */}
          <div className="flex border-b border-slate-200 mb-4 sticky top-0 bg-white z-20">
            <button
              onClick={() => setActiveTab('packages')}
              className={`flex-1 py-2.5 text-[10px] font-bold border-b-2 text-center transition-colors ${
          ...
          {activeTab === 'packages' && (
            <div className="space-y-3 pb-6">
              <div className="text-[11px] text-slate-500 mb-2 font-medium">
                Wybierz pakiet ustandaryzowany do rezerwacji:
              </div>
                  ...
                  >
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className={`text-xs mt-0.5 ${isSelected ? 'text-brand-700' : 'text-slate-500'}`}>Czas: {pkg.durationHours}h • Min. {pkg.minGuests} osób</p>
                      </div>
          ...
          {activeTab === 'menu' && (
            <div className="space-y-4 pb-6">
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 flex items-center justify-between shadow-sm">
                <span className="text-[11px] text-slate-700 font-bold">Pakiet: {selectedPackage.name}</span>
                <span className="text-[11px] font-black text-brand-700">{selectedPackage.pricePerPerson} zł/os.</span>
              </div>
          ...
          {activeTab === 'dates' && (
            <div className="space-y-3 pb-6">
              <div className="text-[11px] text-slate-500 font-medium">
                Wolne soboty potwierdzone przez lokal:
              </div>
              <div className="grid grid-cols-2 gap-2">
                {venue.availableDates.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-[9px] text-slate-500 font-bold uppercase tracking-tight">Wolna Sobota</div>
              ...
              {venue.blockedDates.length > 0 && (
                <div className="mt-4 pt-3 border-t border-slate-200">
                  <span className="text-[9px] uppercase font-bold text-rose-600 tracking-wider block mb-2">
                    Terminy Zajęte
                  </span>
                  <div className="flex flex-wrap gap-2">
                    {venue.blockedDates.map(bd => (
                      <span key={bd} className="bg-slate-50 text-slate-400 line-through text-[10px] px-2.5 py-1 rounded-lg border border-slate-200 font-medium">
                        {bd}
                      </span>
                    ))}
                  </div>
          ...
          {activeTab === 'about' && (
            <div className="space-y-4 pb-6 text-xs text-slate-700">
              ...
              <div>
                <h4 className="font-extrabold text-slate-900 text-xs uppercase tracking-wider mb-2">Wyposażenie i udogodnienia</h4>
                <div className="grid grid-cols-2 gap-2">
                  {venue.amenities.map((am, idx) => (
                    <div key={idx} className="bg-white p-2.5 rounded-xl border border-slate-200 flex items-center gap-2 shadow-sm">
                      ...
                      <span className="text-[11px] font-medium">{am}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-100 space-y-1 shadow-sm">
                <h4 className="font-bold text-slate-900 text-[10px] uppercase">Zasady anulacji</h4>
                <p className="text-slate-500 text-[11px]">{venue.cancellationPolicy}</p>
              </div>
            </div>
          )}
        </div>
      ...
      {/* Footer Action Bar - Fixed height, always at bottom */}
      <div className="flex-none bg-white/95 backdrop-blur-md p-3 border-t border-slate-200 flex items-center gap-3 shadow-[0_-8px_20px_rgba(0,0,0,0.05)] z-40">
        <div className="flex-1 min-w-0">
          <span className="text-[9px] text-slate-500 uppercase font-bold block truncate">Wybrano: {selectedPackage.name}</span>
          <div className="text-lg font-black text-slate-900 flex items-baseline gap-1">
            <span>{selectedPackage.pricePerPerson} zł</span>
            <span className="text-[10px] font-normal text-slate-500">/ os.</span>
          </div>
        </div>

        <button
          onClick={() => onStartBooking(venue, selectedPackage, selectedDate)}
          className="flex-1 py-3.5 px-4 bg-gradient-to-r from-brand-600 to-brand-500 hover:from-brand-500 hover:to-brand-400 text-white font-black text-xs rounded-xl shadow-lg shadow-brand-500/20 active:scale-95 transition-all flex items-center justify-center gap-2 uppercase tracking-wide"
        >
