...
export const MobileAuthModal: React.FC<Props> = ({ user, onLogin, onLogout, onClose }) => {
  ...
  return (
    <div className="absolute inset-0 z-40 bg-black/70 backdrop-blur-md flex items-end justify-center p-0 animate-fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-t-3xl w-full p-6 shadow-2xl relative">
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-slate-400 hover:text-slate-100 p-2 rounded-full bg-slate-800 shadow-sm"
        >
          <X className="w-5 h-5" />
        </button>
        ...
        <div className="text-center mb-6">
          ...
          <h3 className="text-xl font-bold text-slate-100">Logowanie do PartySpot</h3>
        ...
        {user ? (
          <div className="space-y-4">
            <div className="bg-slate-800/60 border border-brand-700/50 rounded-2xl p-4 flex items-center gap-3 shadow-sm">
              {user.avatar ? (
                <img src={user.avatar} alt={user.name} className="w-12 h-12 rounded-full object-cover border border-brand-400" />
              ) : (
                <div className="w-12 h-12 rounded-full bg-brand-900/40 text-brand-600 font-bold flex items-center justify-center text-lg">
                  {user.name.charAt(0)}
                </div>
              )}
              <div className="flex-1 min-w-0">
                ...
                <span className="inline-block mt-1 px-2 py-0.5 text-[10px] font-semibold bg-brand-900/40 text-brand-300 rounded-md border border-brand-700/50">
            ...
            <button
              onClick={onLogout}
              className="w-full py-3 bg-slate-800 hover:bg-slate-700 text-rose-600 font-semibold rounded-xl text-sm transition-colors shadow-sm"
            >
              Wyloguj się
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-3 px-4 bg-slate-800 hover:bg-slate-700 text-slate-100 font-semibold rounded-xl text-sm flex items-center justify-between shadow-sm border border-slate-600 transition-transform active:scale-[0.98]"
            >
            ...
            <div className="relative my-4">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="bg-slate-900 px-2 text-slate-400 font-semibold">Dla Właściciela Lokalu</span>
              </div>
            </div>
            ...
            <button
              ...
              className="w-full py-3 px-4 bg-brand-900/40 hover:bg-brand-100 text-brand-300 font-semibold rounded-xl text-sm flex items-center justify-between border border-brand-700/50 transition-transform active:scale-[0.98] shadow-sm"
            >
              <div className="flex items-center gap-3">
