      ...
      <div className="px-4 mb-4">
        <div className="flex items-center justify-between mb-2">
          <div>
            ...
            <h1 className="text-xl font-extrabold text-slate-100 tracking-tight">Znajdź idealny lokal</h1>
          </div>
          <button
            onClick={onOpenAIChat}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gradient-to-r from-amber-500/10 to-brand-500/10 border border-amber-500/30 text-amber-300 text-xs font-bold shadow-xs active:scale-95 transition-transform"
          >
        ...
        <div className="bg-slate-900 border border-slate-700 rounded-2xl p-3 shadow-md space-y-2.5">
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-2 bg-slate-800/60 rounded-xl px-3 py-2 border border-slate-700">
              <MapPin className="w-4 h-4 text-brand-600 shrink-0" />
              <select
                value={filters.city}
                onChange={(e) => onFilterChange({ ...filters, city: e.target.value })}
                className="bg-transparent text-xs font-semibold text-slate-200 w-full outline-none cursor-pointer"
              >
                {cities.map(c => <option key={c} value={c} className="bg-slate-900 text-slate-200">{c}</option>)}
              </select>
            </div>
            ...
            <div className="flex items-center gap-2 bg-slate-800/60 rounded-xl px-3 py-2 border border-slate-700">
              ...
              <select
                ...
                className="bg-transparent text-xs font-semibold text-slate-200 w-full outline-none cursor-pointer"
              >
                {eventTypes.map(e => <option key={e.id} value={e.id} className="bg-slate-900 text-slate-200">{e.label}</option>)}
              </select>
            </div>
          </div>
...
{/* 
          ...
          <div className="grid grid-cols-2 gap-2">
            <div className="flex items-center gap-2 bg-slate-800/60 rounded-xl px-3 py-2 border border-slate-700">
              ...
              <input
                ...
                className="bg-transparent text-xs font-semibold text-slate-200 w-full outline-none"
              />
            </div>
            <div className="flex items-center gap-2 bg-slate-800/60 rounded-xl px-3 py-2 border border-slate-700">
              <span className="text-xs font-bold text-amber-400 shrink-0">Goście:</span>
              <input
                type="number"
                placeholder="np. 80"
                value={filters.guests || ''}
                onChange={(e) => onFilterChange({ ...filters, guests: parseInt(e.target.value) || 0 })}
                className="bg-transparent text-xs font-semibold text-slate-200 w-full outline-none placeholder-slate-500"
              />
            </div>
          </div>
        </div>
      ...
      <div className="px-4 mb-4">
        <div
          onClick={onOpenAIChat}
          className="bg-gradient-to-r from-amber-950/80 via-rose-950/50 to-orange-950/60 border border-amber-600/50 rounded-2xl p-3.5 flex items-center justify-between cursor-pointer hover:border-amber-400 transition-all shadow-xs"
        >
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-amber-500/15 flex items-center justify-center text-amber-400 shrink-0 shadow-xs">
              <Sparkles className="w-5 h-5 animate-pulse" />
            </div>
            <div>
              <h4 className="text-xs font-extrabold text-amber-200">Asystent Konwersacyjny AI</h4>
              <p className="text-[11px] text-slate-400 font-medium">„Szukam sali na 80 osób pod Krakowem w czerwcu do 250 zł/os”</p>
            </div>
          </div>
        ...
        {filteredVenues.map((venue) => {
          ...
          return (
            <div
              ...
              className="bg-slate-900 border border-slate-700 hover:border-brand-400 rounded-2xl overflow-hidden shadow-md hover:shadow-xl transition-all duration-200 cursor-pointer active:scale-[0.99] group relative"
            >
              <div className="relative h-48 w-full overflow-hidden bg-slate-800">
                ...
                <button
                  ...
                  className="absolute top-3 right-3 w-8 h-8 rounded-full bg-black/70 backdrop-blur-md flex items-center justify-center text-slate-400 hover:text-rose-600 transition-colors shadow-sm"
                >
                  <Heart className={`w-4 h-4 ${isFav ? 'fill-rose-500 text-rose-500' : ''}`} />
                </button>

                <div className="absolute bottom-3 left-3 flex items-center gap-1 bg-slate-900/90 backdrop-blur-md px-2.5 py-1 rounded-lg border border-slate-700 text-xs font-bold text-amber-400 shadow-sm">
                  <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-500" />
                  <span>{venue.rating}</span>
                  <span className="text-[10px] font-normal text-slate-400">({venue.reviewCount})</span>
                </div>
              ...
              <div className="p-4">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-base font-extrabold text-slate-100 group-hover:text-brand-600 transition-colors">
                      {venue.name}
                    </h3>
                    <div className="flex flex-wrap gap-x-1.5 gap-y-0.5 mt-0.5 mb-1">
                      {venue.highlights.slice(0, 3).map((h, i) => (
                        <React.Fragment key={i}>
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-tight">{h}</span>
                    ...
                    {dist !== null && (
                      <p className="text-[11px] text-brand-300 font-semibold mt-1">
                ...
                <div className="mt-3 flex items-center gap-3 text-xs text-slate-300 border-t border-b border-slate-700 py-2">
                ...
                <div className="mt-2.5 flex flex-wrap gap-1.5">
                  {venue.amenities.slice(0, 3).map((amenity, i) => (
                    <span key={i} className="bg-slate-800 text-slate-300 text-[10px] font-medium px-2 py-0.5 rounded-md border border-slate-700">
                      {amenity}
                    </span>
                  ))}
                  {venue.amenities.length > 3 && (
                    <span className="text-[10px] text-slate-400 self-center font-medium">+ {venue.amenities.length - 3}</span>
                  )}
                </div>
                ...
                <div className="mt-3.5 pt-2 flex items-center justify-between gap-2">
                  ...
                  >
                    ...
                    {isComparing && <Check className="w-3.5 h-3.5 text-amber-400 ml-1" />}
                  </button>
