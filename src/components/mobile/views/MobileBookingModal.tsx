...
export const MobileBookingModal: React.FC<Props> = ({
  ...
  return (
    <div className="absolute inset-0 z-40 bg-black/70 backdrop-blur-md overflow-y-auto p-3 flex items-end justify-center animate-fade-in">
      <div className="bg-slate-900 border border-slate-700 rounded-t-3xl w-full p-5 shadow-2xl relative max-h-[90%] overflow-y-auto">
        <button
          onClick={onClose}
        ...
        <div className="mb-4">
          ...
          <h2 className="text-xl font-extrabold text-slate-100">{venue.name}</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs">
          ...
          <div>
            ...
            <div className="space-y-2">
              {venue.packages.map(pkg => (
                ...
                >
                  <div>
                    <span className={`text-xs block ${selectedPackage.id === pkg.id ? 'text-brand-200' : 'text-slate-200'}`}>{pkg.name}</span>
                    <span className="text-[10px] font-normal text-slate-400">Czas: {pkg.durationHours}h</span>
                  </div>
                  <span className={`text-sm font-extrabold ${selectedPackage.id === pkg.id ? 'text-brand-300' : 'text-brand-600'}`}>{pkg.pricePerPerson} zł/os.</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-2">
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-800/60 border border-slate-700 rounded-xl p-2.5 text-slate-200 font-medium outline-none focus:border-brand-500 shadow-sm"
              >
          ...
          <div className="border-t border-slate-700 pt-3 space-y-2">
            ...
            <input
              ...
              className="w-full bg-slate-800/60 border border-slate-700 rounded-xl p-2.5 text-slate-200 outline-none focus:border-brand-500 shadow-sm placeholder-slate-500"
            />
            <div className="grid grid-cols-2 gap-2">
              <input
                type="email"
          ...
          <div className="bg-slate-800/60 border border-brand-700/50 rounded-2xl p-3.5 space-y-1.5 shadow-sm">
            <div className="flex justify-between text-slate-400 text-[11px]">
              <span>Kalkulacja ({guestsCount} os. × {selectedPackage.pricePerPerson} zł):</span>
              <span className="font-bold text-slate-100">{estimatedTotal.toLocaleString()} zł</span>
            </div>
            ...
            <div className="mt-2 pt-2 border-t border-slate-700 text-[10px] text-slate-400 flex items-start gap-1.5">
